/* ═══════════════════════════════════════════════════════════
   HesabKhata Enterprise Pro v7.0
   Ultimate Professional Edition
   ═══════════════════════════════════════════════════════════ */

import { initializeApp } from "https://www.gstatic.com/firebasejs/10.12.2/firebase-app.js";
import {
  getDatabase, ref, push, set, onValue, remove, update, get, query, orderByChild, limitToLast
...
window.formatDateBn = (dateStr) => {
  try { return new Date(dateStr).toLocaleDateString('bn-BD', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }); }
...
window.logActivity = (action, details, amount = null, extraData = {}) => {
  ...
  try {
    localStorage.setItem('hk_activity_' + (AppState.currentUser?.uid || 'guest'), JSON.stringify(AppState.activityLog));
...
window.renderActivityLogAdvanced = () => {
  ...
  Object.keys(grouped).sort().reverse().forEach(date => {
    ...
    header.innerHTML = `<i class="fas fa-calendar-day text-primary"></i><strong>${escapeHtml(window.formatDateBn(date))}</strong><span class="tag tag-primary ms-auto">${dayItems.length} টি</span>${dayTotal > 0 ? `<span class="tag tag-success">মোট: ৳${dayTotal.toFixed(2)}</span>` : ''}`;
    ...
    dayItems.forEach(a => {
      ...
      item.innerHTML = `
        ...
        <div class="timeline-content">
          <div class="timeline-title"><span>${escapeHtml(a.action)}</span>${a.amount ? `<span class="timeline-amount ${amountClass}">৳${a.amount.toFixed(2)}</span>` : ''}</div>
...
window.saveUserSettings = async () => {
  ...
  try { await update(ref(db, 'users/' + AppState.currentUser.uid), { settings: AppState.userSettings }); }
...
function syncSettingsUI() {
  ...
  const radiusEl = $('radiusSlider'), fontEl = $('fontSlider');
  if (radiusEl) radiusEl.value = s.radius;
  if (fontEl) fontEl.value = s.fontSize;
  setText('radiusValue', s.radius); setText('fontValue', s.fontSize);
  const map = {
    compactMode: 'compactMode', animationsToggle: 'animations', sidebarCollapse: 'sidebarCollapsed',
    notifStock: 'notifStock', notifDue: 'notifDue', notifDaily: 'notifDaily',
    soundEffect: 'soundEffect', saveHistory: 'saveHistory', autoLogout: 'autoLogout', sessionTrack: 'sessionTrack'
  };
...
window.toggleSidebarCollapse = (v) => { AppState.userSettings.sidebarCollapsed = v; document.body.classList.toggle('sidebar-collapsed', v); saveUserSettings(); };
...
window.checkPasswordStrength = () => {
  ...
  if (p.match(/[0-9]+/)) s++;
  if (p.match(/[$@#&!]+/)) s++;
  if (p.length === 0) { bar.style.width = '0%'; bar.className = 'password-strength-bar'; txt.textContent = 'পাসওয়ার্ড দিন'; txt.style.color = 'var(--text-muted)'; }
...
window.login = async () => {
  ...
  try {
    ...
    } catch (blockCheckErr) { console.warn('Block check skipped (rules):', blockCheckErr.code); }
    ...
    if (found) {
      ...
      if (status === 'Blocked') {
        ...
        if (!blockUntil || blockUntil > new Date()) {
          ...
          await Swal.fire({
            icon: 'error', title: 'অ্যাকাউন্ট ব্লক করা হয়েছে',
            html: `<div style="text-align:left;font-size:0.9rem;"><p><strong>কারণ:</strong> ${escapeHtml(found.blockReason || 'অ্যাডমিন কর্তৃক ব্লক')}</p><p><strong>আনব্লক হবে:</strong> ${escapeHtml(untilText)}</p></div>`,
      ...
      if (status === 'Suspended') {
        showLoader(false); btn.disabled = false;
        btn.innerHTML = '<i class="fas fa-sign-in-alt me-2"></i>নিরাপদ লগইন';
        await Swal.fire({ icon: 'error', title: 'অ্যাকাউন্ট সাসপেন্ড', text: 'অ্যাডমিনের সাথে যোগাযোগ করুন।', confirmButtonText: 'ঠিক আছে' });
        return;
      }
    }
...
window.register = async () => {
  ...
  try {
    ...
    await set(histRef, {
      id: histRef.key, date: now.toISOString().split('T')[0], time: getTimeBn(now),
      timestamp: now.getTime(), ip: ipInfo.ip || '—', location: ipInfo.location || '—',
      device: device.device, browser: device.browser, os: device.os, action: 'রেজিস্ট্রেশন'
    });
...
onAuthStateChanged(auth, async (user) => {
  ...
  if (!AppState.impersonatingUser) {
    (async () => {
      try {
        const now = new Date();
        const newLoginEntry = {
          date: now.toISOString().split('T')[0], time: getTimeBn(now),
          timestamp: now.getTime(), ip: ipInfo.ip || '—', location: ipInfo.location || '—',
          device: device.device, browser: device.browser, os: device.os, action: 'লগইন'
        };
      ...
      } catch (e) { console.warn('Login history save failed:', e.code); }
    })();
  }
  ...
  if (isAdmin) { try { loadAllUsers(); } catch (e) { showToast('warning', 'অ্যাডমিন প্যানেল লোড হয়নি', 'Rules চেক করুন'); } }
...
window.renderProductCards = () => {
  ...
  if (f.length === 0) {
    c.innerHTML = `<div class="col-12"><div class="empty-state"><i class="fas fa-boxes"></i><h5>কোনো পণ্য নেই</h5><p>নতুন পণ্য যোগ করুন</p><button class="btn btn-gradient" data-bs-toggle="modal" data-bs-target="#addProductModal"><i class="fas fa-plus"></i>নতুন পণ্য</button></div></div>`;
    return;
  }
  const fragment = document.createDocumentFragment();
  f.forEach(p => {
...
window.renderInventoryTable = () => {
  ...
  t.innerHTML = ''; t.appendChild(fragment);
};
...
window.renderCustomerCards = () => {
  ...
  if (f.length === 0) {
    c.innerHTML = `<div class="col-12"><div class="empty-state"><i class="fas fa-users"></i><h5>কোনো কাস্টমার নেই</h5><button class="btn btn-gradient" data-bs-toggle="modal" data-bs-target="#addCustomerModal"><i class="fas fa-plus"></i>নতুন কাস্টমার</button></div></div>`;
    return;
  }
  const fragment = document.createDocumentFragment();
  f.forEach(cust => {
...
window.resetExpenseFilters = () => {
  ['expenseSearchFilter','expenseDateFrom','expenseDateTo'].forEach(id => { const el = $(id); if (el) el.value = ''; });
...
window.exportAllData = () => {
  const backup = {
    ...
    version: '7.0',
    user: {
      email: AppState.currentUser.email, uid: AppState.currentUser.uid,
      fullName: AppState.currentUserFullName, shopName: AppState.currentUserShopName,
      address: AppState.currentUserAddress, role: AppState.currentUserRole
    },
...
window.renderInvoices = (sales) => {
  ...
  if (!c) return;
  if (sales.length === 0) { c.innerHTML = `<div class="col-12"><div class="empty-state"><i class="fas fa-file-invoice-dollar"></i><h5>কোনো ইনভয়েস নেই</h5></div></div>`; return; }
  const fragment = document.createDocumentFragment();
...
window.renderCustomerSelect = () => {
  ...
  AppState.customers.forEach(c => { s.innerHTML += `<option value="${c.id}" data-due="${c.due || 0}">${escapeHtml(c.name)} (বাকি:৳${c.due || 0})</option>`; });
...
window.clearCart = async () => {
  ...
  if (ok) { AppState.cart = []; renderCart(); showToast('info', 'কার্ট খালি'); }
};
...
window.completeSale = async () => {
  if (AppState.cart.length === 0) { showToast('warning', 'সতর্কতা', 'কার্ট খালি'); return; }
  const s = $('customerSelect');
  const cId = s.value;
  ...
  try {
    ...
    logActivity('বিক্রয়', `${cName} - ${AppState.cart.map(i => i.name).join(', ')}`, total, { customerName: cName, invoiceNo });
    AppState.cart = [];
    renderCart();
    $('paidAmount').value = '';
...
window.renderMasterList = () => {
  ...
  if (rows.length === 0) { tb.innerHTML = `<tr><td colspan="${headers.length}" class="text-center text-muted py-4">কোনো ডেটা নেই</td></tr>`; return; }
  const fragment = document.createDocumentFragment();
  rows.forEach(r => {
    const tr = document.createElement('tr');
...
function loadAllUsers() {
  ...
  const unsub = onValue(ref(db, 'users'), (snap) => {
    ...
    try { populateAdminUserDropdown(users); } catch (e) {}
    try { renderAdminUsersList(); } catch (e) {}
    try { renderAdminUserStats(users); } catch (e) {}
  }, (err) => {
    console.error('❌ loadAllUsers permission error:', err.code);
    showToast('warning', 'অ্যাডমিন ডেটা লোড হয়নি', 'Firebase Rules চেক করুন');
  });
  AppState.unsubscribers.push(unsub);
}
...
window.renderAdminUsersList = () => {
  ...
  users.forEach(u => {
    ...
    tr.innerHTML = `
      ...
      <td>${cur ? '<span class="text-muted small">—</span>' : `
...
window.impersonateUser = async () => {
  if (!AppState.selectedAdminUser) { showToast('warning', 'সতর্কতা', 'আগে ইউজার নির্বাচন করুন'); return; }
  const ok = await showConfirm('ইউজার হিসেবে কাজ করবেন?', `${AppState.selectedAdminUser.fullName || AppState.selectedAdminUser.email} এর অ্যাকাউন্টে ঢুকবেন।`, { type: 'warning', okText: 'হ্যাঁ, ঢুকুন' });
  if (!ok) return;
...
window.switchViewDataTab = async (type, el) => {
  ...
  const data = snap.val() ? Object.values(snap.val()) : [];
  if (data.length === 0) { c.innerHTML = `<div class="empty-state"><i class="fas fa-inbox"></i><h5>কোনো ডেটা নেই</h5></div>`; return; }
  if (type === 'sales') {
...
window.adminLoginHistory = async () => {
  ...
  Swal.fire({
    ...
    width: 600, confirmButtonText: 'ঠিক আছে'
  });
};
...
window.adminResetPassword = async () => {
  if (!AppState.selectedAdminUser) { showToast('warning', 'সতর্কতা', 'আগে ইউজার নির্বাচন করুন'); return; }
  const ok = await showConfirm('পাসওয়ার্ড রিসেট?', `${AppState.selectedAdminUser.email} এ রিসেট লিংক পাঠানো হবে।`, { type: 'warning' });
  if (!ok) return;
  try {
...
window.confirmAdminBlock = async () => {
  ...
  try {
    ...
    logActivity('অ্যাডমিন: ইউজার ব্লক', `${AppState.selectedAdminUser.email} — ${unit === 'forever' ? 'চিরতরে' : dur + ' ' + unit}`, null, { reason });
...
window.adminUnblockUser = async () => {
  if (!AppState.selectedAdminUser) { showToast('warning', 'সতর্কতা', 'আগে ইউজার নির্বাচন করুন'); return; }
  const ok = await showConfirm('আনব্লক করবেন?', `${AppState.selectedAdminUser.fullName || AppState.selectedAdminUser.email} আবার সক্রিয় হবেন।`, { type: 'success' });
  if (!ok) return;
  showLoader(true);
  try {
...
window.adminResetUserData = async () => {
  if (!AppState.selectedAdminUser) { showToast('warning', 'সতর্কতা', 'আগে ইউজার নির্বাচন করুন'); return; }
  const ok = await showConfirm('ডেটা রিসেট করবেন?', `${AppState.selectedAdminUser.fullName || AppState.selectedAdminUser.email} এর সব ডেটা মুছে যাবে (ইউজার থাকবে)।`, { type: 'danger', danger: true });
  if (!ok) return;
  showLoader(true);
  try {
...
window.adminDeleteUser = async () => {
  if (!AppState.selectedAdminUser) { showToast('warning', 'সতর্কতা', 'আগে ইউজার নির্বাচন করুন'); return; }
  const ok = await showConfirm('সম্পূর্ণ ডিলিট?', `${AppState.selectedAdminUser.fullName || AppState.selectedAdminUser.email} ফায়ারবেস থেকে মুছে যাবে — ফেরানো যাবে না!`, { type: 'danger', danger: true, okText: 'হ্যাঁ, ডিলিট' });
  if (!ok) return;
  showLoader(true);
  try {
...
window.adminDeleteUserById = async (uid, email) => {
  if (uid === AppState.currentUser.uid) { showToast('warning', 'সতর্কতা', 'নিজেকে ডিলিট করতে পারবেন না'); return; }
...
window.adminCreateUser = () => {
  ...
  $('newUserRole').value = 'Staff'; $('newUserShop').value = ''; $('newUserAddress').value = '';
...
window.confirmAdminCreateUser = async () => {
  ...
  try {
    ...
    await Swal.fire({
      ...
      html: `<div style="text-align:left;"><p><strong>নাম:</strong> ${escapeHtml(name)}</p><p><strong>ইমেইল:</strong> ${escapeHtml(email)}</p><p><strong>পাসওয়ার্ড:</strong> <code style="background:#f1f5f9;padding:4px 8px;border-radius:6px;">${escapeHtml(password)}</code></p><p class="text-warning small"><strong>নোট:</strong> আপনি লগ আউট হয়েছেন। আবার লগইন করুন।</p></div>`,
      confirmButtonText: 'ঠিক আছে'
    });
...
window.globalSearch = debounce(() => {
  ...
  let html = '';
  if (ps.length) html += `<div class="text-start mb-2"><strong>📦 পণ্য:</strong><br>${ps.slice(0, 5).map(p => `${escapeHtml(p.name)} — স্টক: ${p.qty}, মূল্য: ৳${p.sellPrice}`).join('<br>')}</div>`;
  if (cs.length) html += `<div class="text-start mb-2"><strong>👥 কাস্টমার:</strong><br>${cs.slice(0, 5).map(c => `${escapeHtml(c.name)} — বাকি: ৳${c.due}`).join('<br>')}</div>`;
  if (ss.length) html += `<div class="text-start"><strong>🧾 বিক্রয়:</strong><br>${ss.slice(0, 5).map(s => `${escapeHtml(s.invoiceNo)} — ৳${s.totalAmount}`).join('<br>')}</div>`;
...
window.toggleFullscreen = () => {
  if (!document.fullscreenElement) {
    document.documentElement.requestFullscreen().catch(() => showToast('warning', 'ফুলস্ক্রিন ব্যর্থ'));
  } else {
...
window.openBarcodeScanner = () => {
  ...
  if (typeof ZXing === 'undefined') {
    showToast('warning', 'বারকোড লাইব্রেরি লোড হয়নি', 'ম্যানুয়ালি লিখুন');
    return;
  }
  $('manualBarcode').value = '';
  setTimeout(() => {
    try {
      ...
      AppState.barcodeReader.decodeFromVideoDevice(null, 'barcodeReader', (result, err) => {
        if (result) {
...
window.searchByBarcode = () => {
  ...
  if (!code) { showToast('warning', 'সতর্কতা', 'বারকোড লিখুন'); return; }
  ...
  if (item) {
    ...
    showToast('error', 'পাওয়া যায়নি', `বারকোড: ${code}`);
  }
};
...
const calcState = {
  ...
  memory: 0, angleMode: 'DEG', history: [], expression: ''
};
...
window.calcAction = (action) => {
  ...
  switch (action) {
    case 'clear':
      calcState.current = '0'; calcState.previous = null;
      calcState.operator = null; calcState.waitingForOperand = false; calcState.expression = '';
      break;
    case 'backspace':
    ...
    case 'copy':
      ...
      break;
    case 'paste':
      navigator.clipboard.readText().then(t => { if (!isNaN(parseFloat(t))) { calcState.current = t; updateCalcDisplay(); } }).catch(() => showToast('warning', 'পেস্ট ব্যর্থ'));
      break;
  }
  updateCalcDisplay();
};
...
function performCalc() {
  ...
  switch (calcState.operator) {
    ...
    case 'pow': result = Math.pow(prev, cur); break;
    default: result = cur;
  }
...
function getOpSymbol(op) { return { add: '+', subtract: '−', multiply: '×', divide: '÷', pow: '^' }[op] || ''; }
...
window.calcFunc = (fn) => {
  ...
  try {
    switch (fn) {
      ...
      case 'fact':
        if (cur < 0 || !Number.isInteger(cur)) { showToast('error', 'ভুল', 'শুধু ধনাত্মক পূর্ণসংখ্যা'); return; }
        if (cur > 170) { showToast('error', 'অনেক বড়', 'সীমা ১৭০'); return; }
        result = 1; for (let i = 2; i <= cur; i++) result *= i;
        break;
      case 'pow':
        calcState.previous = cur; calcState.operator = 'pow';
        calcState.waitingForOperand = true;
...
window.clearCalcHistory = () => { calcState.history = []; renderCalcHistory(); showToast('info', 'হিস্ট্রি মুছে ফেলা হয়েছে'); };
...
document.addEventListener('keydown', (e) => {
  ...
  if ((e.ctrlKey || e.metaKey) && e.key === 'k') { e.preventDefault(); return; }
  if (e.key === 'Enter') {
...
window.addEventListener('online', () => {
  ...
  if (banner) banner.style.display = 'none';
  showToast('success', 'অনলাইন', 'ইন্টারনেট সংযোগ ফিরে এসেছে');
});
window.addEventListener('offline', () => {
  const banner = $('offlineBanner');
  if (banner) banner.style.display = 'block';
  showToast('warning', 'অফলাইন', 'ইন্টারনেট সংযোগ নেই');
});
...
(function initCommandPalette() {
  const commands = [
    ...
    { id: 'barcode', title: 'বারকোড স্ক্যানার', icon: 'fas fa-barcode', action: () => openBarcodeScanner() },
    { id: 'add-product', title: 'নতুন পণ্য যোগ করুন', icon: 'fas fa-plus-circle', action: () => new bootstrap.Modal($('addProductModal')).show() },
    { id: 'add-customer', title: 'নতুন কাস্টমার যোগ করুন', icon: 'fas fa-user-plus', action: () => new bootstrap.Modal($('addCustomerModal')).show() },
    { id: 'add-expense', title: 'নতুন খরচ যোগ করুন', icon: 'fas fa-receipt', action: () => new bootstrap.Modal($('addExpenseModal')).show() },
  ...
  overlay.innerHTML = `<div class="command-palette"><input type="text" class="command-palette-input" id="commandPaletteInput" placeholder="🔍 কমান্ড খুঁজুন... (যেমন: বিক্রয়, পণ্য, সেটিংস)"><div class="command-palette-list" id="commandPaletteList"></div></div>`;
  ...
  function renderCommands() {
    if (filteredCommands.length === 0) {
      list.innerHTML = `<div style="text-align:center;padding:32px;color:var(--text-muted);"><i class="fas fa-search" style="font-size:2rem;opacity:0.3;margin-bottom:10px;display:block;"></i><p>কিছু পাওয়া যায়নি</p></div>`;
      return;
    }
    list.innerHTML = filteredCommands.map((cmd, i) => `<div class="command-palette-item ${i === activeIndex ? 'active' : ''}" data-index="${i}"><i class="${cmd.icon}"></i><span>${cmd.title}</span>${cmd.key ? `<kbd>${cmd.key}</kbd>` : ''}</div>`).join('');
    list.querySelectorAll('.command-palette-item').forEach(item => {
      ...
      item.addEventListener('mouseenter', () => { activeIndex = parseInt(item.dataset.index); updateActive(); });
    });
  }

  function updateActive() {
    list.querySelectorAll('.command-palette-item').forEach((item, i) => item.classList.toggle('active', i === activeIndex));
  ...
  function closePalette() { overlay.classList.remove('show'); input.value = ''; filteredCommands = [...commands]; activeIndex = 0; }

  input.addEventListener('input', (e) => {
    const q = e.target.value.toLowerCase().trim();
    filteredCommands = q ? commands.filter(c => c.title.toLowerCase().includes(q) || c.id.includes(q)) : [...commands];
    activeIndex = 0; renderCommands();
  });

  input.addEventListener('keydown', (e) => {
    if (e.key === 'ArrowDown') { e.preventDefault(); activeIndex = Math.min(activeIndex + 1, filteredCommands.length - 1); updateActive(); }
    else if (e.key === 'ArrowUp') { e.preventDefault(); activeIndex = Math.max(activeIndex - 1, 0); updateActive(); }
    else if (e.key === 'Enter') { e.preventDefault(); executeCommand(activeIndex); }
    else if (e.key === 'Escape') closePalette();
  });

  overlay.addEventListener('click', (e) => { if (e.target === overlay) closePalette(); });

  window.openCommandPalette = () => { overlay.classList.add('show'); setTimeout(() => input.focus(), 50); renderCommands(); };
...
(function initVoiceInput() {
  ...
  window.attachVoiceInput = (inputId) => {
    ...
    btn.type = 'button'; btn.className = 'voice-input-btn'; btn.title = 'ভয়েস ইনপুট';
    ...
    btn.addEventListener('click', () => {
      ...
      recognition.onstart = () => { isRecording = true; btn.classList.add('recording'); btn.innerHTML = '<i class="fas fa-stop"></i>'; showToast('info', 'শুনছি...', 'বলুন'); };
      ...
      recognition.onerror = () => showToast('error', 'ভয়েস সমস্যা', 'আবার চেষ্টা করুন');
      recognition.onend = () => { isRecording = false; btn.classList.remove('recording'); btn.innerHTML = '<i class="fas fa-microphone"></i>'; };
      recognition.start();
    });
    input.parentNode.insertBefore(wrapper, input);
    wrapper.appendChild(input); wrapper.appendChild(btn);
  };
  ['addProdName','addCustName','addExpDesc','regName','regShopName','regAddress'].forEach(id => {
...
(function initAutoSave() {
  const drafts = [
    { key: 'draft_product_v7', ids: ['addProdName','addProdQty','addProdBuyPrice','addProdSellPrice'] },
    { key: 'draft_customer_v7', ids: ['addCustName','addCustPhone','addCustDue'] },
    { key: 'draft_expense_v7', ids: ['addExpDesc','addExpAmt'] }
  ];
  drafts.forEach(({ key, ids }) => {
    try {
      const saved = localStorage.getItem(key);
      if (saved) { const data = JSON.parse(saved); ids.forEach(id => { const el = document.getElementById(id); if (el && data[id]) el.value = data[id]; }); }
    } catch (e) {}
    ids.forEach(id => {
      ...
      el.addEventListener('input', debounce(() => {
        const data = {};
        ids.forEach(i => { const inp = document.getElementById(i); if (inp && inp.value) data[i] = inp.value; });
        try { if (Object.keys(data).length > 0) localStorage.setItem(key, JSON.stringify(data)); else localStorage.removeItem(key); } catch (e) {}
      }, 500));
    });
  });
  window.clearDrafts = () => {
    ['draft_product_v7','draft_customer_v7','draft_expense_v7'].forEach(k => { try { localStorage.removeItem(k); } catch (e) {} });
  };
})();
...
(function initRipple() {
  document.addEventListener('click', (e) => {
    ...
    ripple.style.cssText = `position:absolute;left:${x}px;top:${y}px;width:${size}px;height:${size}px;border-radius:50%;background:rgba(255,255,255,0.4);pointer-events:none;transform:scale(0);animation:rippleAnim 0.6s ease-out;z-index:1;`;
...
(function initProShortcuts() {
  const shortcuts = {
    ...
    'Alt+6': () => showSection('reports')
  };
  document.addEventListener('keydown', (e) => {
    if (['INPUT','TEXTAREA','SELECT'].includes(e.target.tagName)) return;
    const key = [e.altKey && 'Alt', e.shiftKey && 'Shift', e.ctrlKey && 'Ctrl', e.key].filter(Boolean).join('+');
  ...
  function createSuggestions(inputId, getSuggestions) {
    ...
    input.addEventListener('input', () => {
      ...
      debounceTimer = setTimeout(() => {
        ...
        dropdown.innerHTML = suggestions.map(s => `<div class="suggestion-item" data-value="${escapeHtml(s.value)}"><i class="fas fa-lightbulb" style="color:var(--warning);"></i><span>${escapeHtml(s.label)}</span></div>`).join('');
    ...
    input.addEventListener('focus', () => { if (input.value.length > 0) input.dispatchEvent(new Event('input', { bubbles: true })); });
  }
  createSuggestions('addProdName', (q) => (AppState.inventory || []).filter(i => i.name.toLowerCase().includes(q)).map(i => ({ value: i.name, label: `${i.name} (আগের মূল্য: ৳${i.sellPrice})` })));
  createSuggestions('addCustName', (q) => (AppState.customers || []).filter(c => c.name.toLowerCase().includes(q)).map(c => ({ value: c.name, label: `${c.name} — ${c.phone || 'ফোন নেই'}` })));
  const commonExpenses = ['দোকান ভাড়া','বিদ্যুৎ বিল','পানি বিল','ইন্টারনেট বিল','বেতন','যাতায়াত','প্যাকেজিং','মার্কেটিং','মেরামত','স্টেশনারি'];
  createSuggestions('addExpDesc', (q) => commonExpenses.filter(e => e.toLowerCase().includes(q)).map(e => ({ value: e, label: e })));
})();
